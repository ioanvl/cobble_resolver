...
class HelperText:
    ...
    GENERIC_HELP = (
        ...
        f"\n\n!! - It is highly {c_text("reccomended", bcolors.BOLD)} "
        "(though not necessary) to include the main \"Cobblemon\" mod, and any other "
        f"{c_text("mods (not packs)", bcolors.BOLD)} that add pokemon "
    ...
    OUTPUT_HELP = (
        ...
        f"\n{DefaultNames.REMAINDER_PACK_PREFIX}[R|D]_another_addon_name.."
        "\n\nSuch packs will be there if any of the packs you combined had "
        "extras things other than pokemon, like structures, items, loot tables etc. "
        ...
        f"if [R|D] then in {c_text("both", bcolors.UNDERLINE)}"
    )
